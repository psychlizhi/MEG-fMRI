function matrixplot(data, fMRI_task, varargin)
...
% Preperation
warning off
HCP_ROI = readtable('/home/zhi.li/DCM_BMA/HCP_ROI_label.csv');
% fMRI_task = 'NB_2back';
data_dir = dir(['/home/zhi.li/ID002_NVC/fMRI_DCM/' fMRI_task '.mat']);
load([data_dir.folder '/' data_dir.name]);
eval('fc = NB_2back.fc_name(:,2:3);')
roi = unique(fc);
roi_new = [];
idx_final = [];
for i = 1:length(roi)
...
idx = [idx; length(x)];
xline(x(1, idx([1,end])),'k-');%, 'Color', [0, 0, 0], 'LineWidth',1.5);
yline(y(idx([1,end]), 1),'k-');%,'Color', [0, 0, 0], 'LineWidth',1.5);
xline(x(1, idx([2:(end-1)])),'k-.');%, 'Color', [0, 0, 0]);
yline(y(idx([2:(end-1)]), 1),'k-.');%, 'Color', [0, 0, 0]);
warning on
% % Add color annotation to big regions
% y_raw = gca.XR
% [xx yy] = datc2figc([1 2],[5 6]);
...
x = zeros(size(ytick)) + xl(1) - range(xl)/10;
